...
wasm-page-len = 1 << wasm-page-len-log
...
len-to-class = (len/u32) /u32 {
    ...
    upper-len-log = if {{1 << lower-log} == len-with-free-ptr} lower-len-log else lower-len-log + 1
    if {upper-len-log > class-min-len-log} upper-len-log - class-min-len-log else 0
}

class-to-len = (class/u32) /u32 {
    {1 << {class + class-min-len-log}} - %size-of(u32)
}
