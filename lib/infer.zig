...
fn inferExprInner(
    ...
    switch (expr_data) {
        ...
        .each => {
            ...
            const fun = try inferExpr(c, f, dir_f, .other);
            if (fun != .fun)
                return fail(c, .{ .invalid_call_builtin = .{ .builtin = .each, .args = c.dupe(Repr, &.{ object, fun }) } });

            const fun_local = f.local_data.append(.{ .repr = .{ .@"struct" = fun.fun.closure }, .is_tmp = true });
            emit(c, f, .{ .local_let = fun_local });

            _ = take(c, dir_f).each_begin;
            _ = take(c, dir_f).each_body;
            ...
            switch (object) {
                ...
                .only => |only| {
                    switch (only.*) {
                        ...
                        else => return fail(c, .{ .invalid_call_builtin = .{ .builtin = .each, .args = c.dupe(Repr, &.{ object, fun }) } }),
                    }
                },
                else => return fail(c, .{ .invalid_call_builtin = .{ .builtin = .each, .args = c.dupe(Repr, &.{ object, fun }) } }),
            }
